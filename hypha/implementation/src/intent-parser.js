...
function extractTaste(t) {
  const TASTE = [
    ...
    ['甜', ['甜', '糖水', '甜品', '糕点', '酒酿', '糊米酒', '奶茶', '糖']], // 豆皮为咸鲜早点，不属于甜味词（曾致「豆皮」检索被甜味过滤误伤）
...
export function parseIntent(input = {}) {
  ...
  // 健康饮食语义（W1 · 实事求是）：数据无「健身餐」类目时，检索词精确落 FIT 词本身，
  // 避免整句 0 命中后随便凑；同时把口味约束「清淡」挂上（低脂/高蛋白/健身 → 清淡筛选）。
  const FIT_TERMS = ['健身餐', '减脂餐', '轻食沙拉', '轻食', '沙拉', '鸡胸肉', '水煮菜', '健身', '减脂', '低脂', '高蛋白', '增肌'];
  const fitTerm = FIT_TERMS.find((f) => intent.includes(f));
  let keyword = input.keyword != null ? String(input.keyword) : '';
  if (!keyword && fitTerm) keyword = fitTerm; // 「清淡的健身餐」→ 检索词=「健身餐」（而非整句）
  if (fitTerm && (!taste || !taste.length)) taste = ['清淡']; // extractTaste 无匹配返回 [] 而非 null

...
// 小 fixtures（贴近真实字段）
const M = [
  { id: 'a', name: '老李热干面', zone: '首义', category: '早餐', cuisine: '小吃', mealTime: ['早'], avgPrice: '8', rating: '必吃', signatureDishes: '热干面、豆皮', reason: '过早之王' },
  { id: 'b', name: '川味火锅', zone: '首义', category: '火锅', cuisine: '火锅', mealTime: ['午', '晚'], avgPrice: '120', rating: '推荐', signatureDishes: '牛油锅底', reason: '' },
  { id: 'c', name: '南湖烧烤摊', zone: '南湖', category: '烧烤', cuisine: '烧烤', mealTime: ['晚', '夜宵'], avgPrice: '55', rating: '', signatureDishes: '烤筋子、烤鱼', reason: '宵夜好去处' },
  { id: 'd', name: '全城湘菜馆', zone: '全城', category: '湘菜', cuisine: '湘菜', mealTime: ['午', '晚'], avgPrice: '', rating: '推荐', signatureDishes: '剁椒鱼头', reason: '' },
  { id: 'e', name: '深夜饺子', zone: '首义', category: '小吃宵夜', cuisine: '饺子', mealTime: ['夜宵'], avgPrice: '20', rating: '必吃', signatureDishes: '三鲜饺子', reason: '' }
];
...
const d = distKm(CAMPUS_COORDS.首义, CAMPUS_COORDS.南湖);
ok(d > 5 && d < 12, `首义↔南湖距离在合理范围 (${d.toFixed(1)}km)`);

console.log('# 筛选');
ok(filterMerchants(M, { zone: '首义' }).length === 3, '按 zone=首义 筛出 3 条');
...
ok(filterMerchants(M, { zone: '首义', maxPrice: 50 }).length === 2, '组合：首义 + 人均≤50');
...
const byDist = sortMerchants(M.filter(m => m.zone === '首义'), { sort: 'distance', fromCoord: CAMPUS_COORDS.首义 });

...
## 当前焦点
- 当前版 **V4 · 商户网络 + CPS 分润平台(BFF 后端)(待 Robin 决策/解锁)**;**V3 已全交付**(V3.1 账号体系 / V3.2 收藏+券包 / V3.3 到店核销 CPS 闭环 / V3.4 校园增长+埋点看板 — 均勾选,结论块见 roadmap,验收测试全绿)。V4 首任务 V4.1 BFF 后端需自建后端/支付密钥不进前端/不部署公网,属 Robin 决策项,自动化将标 needs-Robin 并停等。V4.4 数据源口径只读核对切片已完成(实测 allMerchants=857 / merchants.js=625 + 61 组后端重名待治理,见 docs/datasource-reconcile.md);V4.1–V4.3 仍待 Robin 决策。V2 代码层已完成(V2.1~V2.4 已勾选);V1 代码完成,**真实付费 `/agent` 跑量复核待 Robin 在 env 设 `DEEPSEEK_API_KEY`**(沙箱无 Key,已授权 D-20260813-01)——该复核是 Verification 门禁,不阻塞 V4 并行推进。
  - 架构结论已定：**生产环境优先国内 LLM(DeepSeek 付费档),海外模型走后端出海**(详见 D-20260812-01)。push 已授权执行(见 open-threads)。

## 版本路线图（详情 layers/roadmap.md）
- 当前版:**V4 · 商户网络 + CPS 分润平台(BFF 后端)**(待 Robin 决策/解锁:V4.1 BFF 后端需自建后端、支付密钥不进前端、不部署公网);**V3 已全交付**(V3.1 账号体系 / V3.2 收藏+券包 / V3.3 到店核销 CPS 闭环 / V3.4 校园增长+埋点看板,结论块见 roadmap)。V2 代码完成(V2.1~V2.4 已交付);V1 代码完成、真跑复核待 Robin 设 env Key。地基 v0 已完成(数据层+算法层,本地可玩,未 push)。

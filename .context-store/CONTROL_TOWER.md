...
## 当前焦点
- **2026-08-15**：SPEC v2.0 已重构；**S2 数据统一 ✅**（860）；**S3 账号持久化 ✅**（重启旧 JWT 有效）；**S4 收藏跨设备同步 ✅**（JWT 鉴权 + favorites.json + 防越权 + 前端兜底，跨设备用例实测通过）；40 子测试全绿。
- **下一步清单（按序推进，见 SPEC §11）**：**S5 pending 上传治理（/upload/pending 查询 + 治理工具 dry-run + 审计日志）** → S6 对话体验打磨（chat-first）。
